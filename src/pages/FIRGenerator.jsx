      ...
      <div className="workflow-panel">
        <AnimatePresence mode="wait" initial={false}>
          <motion.div key={step} initial={{ opacity: 0, x: 8 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -8 }} transition={{ duration: .16 }}>
            {step === 0 && <><h2 className="workflow-panel__heading">Start with the incident details</h2><p className="workflow-panel__copy">Location and date are optional, but they make the final draft more complete.</p><section className="form-section" aria-labelledby="fir-category-label"><span className="field-label" id="fir-category-label">Incident type</span><div className="pill-group">{CATEGORIES.map((cat) => <button key={cat} type="button" className="choice-pill" aria-pressed={category === cat} onClick={() => setCategory(cat)}>{cat}</button>)}</div></section><div className="field-grid"><div><label className="field-label" htmlFor="fir-location">Location</label><input id="fir-location" className="field-control" type="text" value={location} onChange={(e) => setLocation(e.target.value)} placeholder="e.g. Hyderabad, Telangana" /></div><div><label className="field-label" htmlFor="fir-date">Date</label><input id="fir-date" className="field-control" type="date" value={date} onChange={(e) => setDate(e.target.value)} /></div></div></>}
